...
/* Created on:     3/19/2015 2:45:33 PM                         */
/*==============================================================*/
...
create table muon_tiem (
   ma_phieu             VARCHAR(15)          not null,
   ma_benh              VARCHAR(15)          not null,
   constraint PK_MUON_TIEM primary key (ma_phieu, ma_benh)
);
...
alter table muon_tiem
   add constraint FK_MUON_TIE_RELATIONS_PHIEU_TI foreign key (ma_phieu)
      references phieu_tiem (ma_phieu)
      on delete restrict on update restrict;
...
alter table phieu_hen
   add constraint FK_PHIEU_HE_RELATIONS_PHIEU_TI foreign key (ma_phieu)
      references phieu_tiem (ma_phieu)
      on delete restrict on update restrict;
